...
<link type="text/css" rel="stylesheet"
	href="<c:url value='/css/egovframework/com/ext/msg/bubbleChat.css'/>" />

<title>${pageTitle}chat</title>
<!--WebSocket messenger chat -->
	...
	function processMessage(message) {
		...
		if (jsonData.messageType == "ChatMessage") {
			...
			for (var i = 0; i < jsonData.users.length; i++) {
				if ("${loginVO.name}" != jsonData.users[i]) {
					$('#users')
							.append(
									jsonData.users[i]
											+ "<spring:message code="comExtMsg.webSocketChat.msg1"/>"); //님과 대화중입니다.
			...
			if (first == "false" && other == "") {
				...
				$('#users')
						.append(
								"<spring:message code="comExtMsg.webSocketChat.msg2"/>"); //대화상대가 아무도 없습니다.
			}
		}
	}
	...
	function displaybubble(data) {
		...
		if (data.name == "${loginVO.name}") {
			$('#chat')
					.append(
							data.name
									+ "(나)<br/><div class='bubble right'><span class='tail'>&nbsp;</span>"
									+ data.message + "</div>");
			...
			$('#chat')
					.append(
							data.name
									+ "(상대방)<br/><div class='bubble left'><span class='tail'>&nbsp;</span>"
									+ data.message + "</div>");
		}
	}
...
<body>
	...
	<input type="button" id="leaveBtn" name="leaveBtn" value="나가기" />
</body>
</html>
